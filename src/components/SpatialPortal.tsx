      ...
      <div className="glass-dark text-gray-100 rounded-2xl p-4.5 space-y-3.5 overflow-hidden shadow-2xl relative" id={`api_console_${parcelId}`}>
        ...
        {!apiLoading && !apiLoaded && (
          <div className="py-4 text-center space-y-3">
            <p className="text-xs text-gray-300 leading-normal max-w-sm mx-auto">
              이 유휴공간의 실시간 용도구역 저촉, 건폐율제한 및 지적 표준 공공 데이터를 모의조회 하겠습니까?
            </p>
            <button
              onClick={handleTriggerApi}
              className="px-5 py-2.5 btn-premium-jade text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 mx-auto transition active:scale-95 shadow-md cursor-pointer"
        ...
        {apiLoaded && (
          <div className="space-y-3 pt-1 animate-in fade-in-50 duration-250">
            ...
            <div className="bg-emerald-950/40 p-2.5 rounded-xl border border-brand-green/20 text-[10px] text-emerald-300 leading-relaxed flex gap-1.5">
              ...
              <span>
                <strong>대부 자정 통보:</strong> 본 부지는 국토계획 상 **'{zoning}'**에 속하며, 철도안전법 저촉 우려에 대한 가상 시뮬레이션을 통과하였습니다. {hasRoadAccess ? '지적도상 완벽한 도로 접합이 확보되어 즉시 행정 신청에 걸림돌이 없습니다.' : '진입로가 미확보(맹지)되어 있으므로 공사 전 평탄화 지원 방안을 지사에서 연계하여 안전하게 이용 가능합니다!'}
              </span>
            </div>
